<div class="min-h-screen bg-white font-mono text-black">
    ...
    <div class="max-w-7xl mx-auto p-6 mt-8">
        ...
        @if (session()->has('message'))
            <div class="mb-6 bg-green-100 border-2 border-black text-black p-4 font-bold shadow-[4px_4px_0px_0px_#000] flex items-center justify-between animate-bounce">
        ...
        <div class="mb-10 border-4 border-black p-6 bg-gray-100 shadow-[8px_8px_0px_0px_#000] relative overflow-hidden">
            <div class="relative z-10">
                <h1 class="text-3xl md:text-5xl font-black uppercase mb-2">DASHBOARD</h1>
                <p class="text-gray-600 font-bold text-lg">Selamat datang kembali! Siap merangkai kata hari ini?</p>
            </div>
            <div class="absolute -right-10 -bottom-10 text-9xl opacity-5 select-none pointer-events-none">✍️</div>
        </div>
        ...
        <div>
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 border-b-4 border-black pb-4 gap-4">
                ...
                {{-- [PERBAIKAN] LINK TOMBOL BUAT NOVEL BARU --}}
                                ...
                                <tr class="hover:bg-orange-50 group transition-colors">
                                    ...
                                    <td class="p-4 align-middle text-center">
                                        @if($book->is_published)
                                            <span class="inline-block bg-[#e97124] text-black border-2 border-black px-3 py-1 text-xs font-black shadow-[2px_2px_0px_0px_#000]">PUBLISHED</span>
                                        @else
                                            <span class="inline-block bg-gray-200 text-gray-500 border-2 border-black px-3 py-1 text-xs font-black border-dashed">DRAFT</span>
                                        @endif
                                    </td>
                                    <td class="p-4 align-middle text-center">
                                    ...
                                    <td class="p-4 align-middle text-right">
                                        <div class="flex justify-end gap-2">
                                            {{-- Tombol Lihat (Opsional, pastikan route book.detail ada) --}}
                                            <a href="{{ route('book.detail', $book->id) }}" class="flex items-center justify-center w-10 h-10 border-2 border-black bg-white hover:bg-black hover:text-white transition-all shadow-[2px_2px_0px_0px_#ccc] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px]" title="Lihat">
                                                👁️
                                            </a>
                                            {{-- Tombol Edit --}}
                                            <a href="{{ route('book.manage', $book->id) }}" class="flex items-center justify-center w-10 h-10 border-2 border-black bg-yellow-300 hover:bg-yellow-400 transition-all shadow-[2px_2px_0px_0px_#000] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px]" title="Edit">
                                                ✏️
                                            </a>
                                            {{-- Tombol Hapus --}}
                                            <button wire:click="deleteBook({{ $book->id }})" wire:confirm="Yakin ingin menghapus novel ini?" class="flex items-center justify-center w-10 h-10 border-2 border-black bg-red-500 text-white hover:bg-red-600 transition-all shadow-[2px_2px_0px_0px_#000] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px]" title="Hapus">
                                                🗑️
                                            </button>
                                        </div>
                                    </td>
